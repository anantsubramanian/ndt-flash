package  {
	...
	public class Protocol {
		...
		/*
			Class aggregating operations that can be performed
			for sending / receiving / reading Protocol messages
		...
		*/
		...
		/*
			...
			@return integeer number of bytes populated
		...
		*/
		public function readn(msgParam:Message, iParamAmount:int):int {
			...
			while(read != iParamAmount) {
				tmp = readBytesAndReturn(ctlSocket, msgParam._yaBody, read, iParamAmount - read);
				if(tmp <= 0) {
					// end of file 
					return read;
				}
				read += tmp;
			} // to be tested - verify working of function readBytesAndReturn
			return read;
		}
		...
		/*
			...
			@param offset
					  Offset value to start
					  
			@param len
					  Length to be read
					  
			@return An integer indicating the number of successfully read bytes
		*/
		
		public function readBytesAndReturn(socket:Socket, bytes:ByteArray, offset:uint, len:uint):int {
		...
		/*
			...
			@return integer with values:
					...
					b) Error - value=1 : error reading ctrl-message length and data type itself,
										 since NDTP-control packet has to be atleast 3 octets long.
